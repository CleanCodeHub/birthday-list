...
CREATE TABLE IF NOT EXISTS birthday_info (
  id VARCHAR(36) PRIMARY KEY,
  birthday_person_name VARCHAR(255) DEFAULT 'Birthday Person',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Insert default birthday info
INSERT INTO birthday_info (id, birthday_person_name)
SELECT UUID(), 'Birthday Person'
WHERE NOT EXISTS (SELECT 1 FROM birthday_info LIMIT 1);
